<template>
	<view>
		...
		<upload-image chooseType="chooseMedia" :show="false" ref="upload" @allComplete="upLoadallComplete" @oneComplete="upLoadoneComplete"></upload-image>
		<!-- #ifndef H5 -->
		<view class="zfb-tk-recorder" v-show="showRecorder"><recorder :show="showRecorder" ref="rec" @recorderStop="recorderStop"></recorder></view>
		<!-- #endif -->
...
export default {
	...
	methods: {
		...
		startRecord() {
			...
			console.log(12355,this.$refs['rec'])
			...
			switch (e.title) {
				case '位置':
					uni.chooseLocation({
						type: 'gcj02',
						success: res => {
							console.log('uni.chooseLocation',res)
							this.sendMsg(JSON.stringify(res), 'LOCATION');
						},
						fail:res=>{
							console.log('getLocation2',res)
						}
					});
					break;
				case '相册':
