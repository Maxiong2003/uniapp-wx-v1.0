		...
		<uni-list-wx>
			<uni-list-wx :border="true">
				...
				<template v-for="(v, i) in toplist" :key="i">
						<uni-list-chat-wx color="#eeeeee" :itemKey="i" :item="v" @longpressItem="longpressItem" :longTapItemKey="longTapItemKey" @click="clickChat(v)" clickable :title="v.nickName" :note="v.content" :time="formatTime(v.time)" badge-positon="left" :badge-text="v.num"></uni-list-chat-wx>
					</template>
					<template v-for="(v, i) in NOTtoplist" :key="i">
						<uni-list-chat-wx color="#fff" :itemKey="i" :item="v" @longpressItem="longpressItem" :longTapItemKey="longTapItemKey" @click="clickChat(v)" clickable :title="v.nickName" :note="v.content" :time="formatTime(v.time)" badge-positon="left" :badge-text="v.num"></uni-list-chat-wx>
					</template>
			</uni-list-wx>
		</uni-list-wx>
...
export default {
	...
	methods: {
		...
		formatTime(time) {
			return this.$fc.getNewDate('time12cn', true, time);
		},
		returnAvatar(text) {
