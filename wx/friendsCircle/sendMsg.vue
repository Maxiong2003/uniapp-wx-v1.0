<template>
	<view class="friendsCircle-send">
		<form @submit="subform">
			...
			<view class="friendsCircle-send-item">
				<upload-image chooseType="chooseMedia" @oneComplete="upLoadoneComplete" ref="upup" v-model:fileList="form.files" :limit="limit"></upload-image>
			</view>
			<view class="friendsCircle-send-item">
...
export default {
	...
	methods: {
		...
		subform(e) {
			...
			var checkRes = this.$imFormCheck.check(formData, rules);
			if (checkRes) {
				this.$http.request({
					url: '/topic/sendTopic',
					method: 'POST',
