<template>
	<view class="upload-image" v-if="show">
		<upload-image-drag :key="key" :rowNum="rows" :imgRadius="imgRadius" :imgSize="imgSize" :imgPadding="imgPadding" :imgList="fileList" @deleteImage="deleteImage" @itemclick="itemclick">
			<view v-if="showAdd && fileList.length < limit" @tap="chooseTap" :style="{ width: imgSize + 'rpx', height: imgSize + 'rpx',padding:imgPadding+'rpx' }" class="upload-image-item-slot">
				<view class="upload-image-item-slotIcon">
...
<script>
import UploadImageDrag from './upload-image-drag.vue';
...
.upload-image-item-slot{
	box-sizing: border-box;
}
.upload-image-item-slotIcon {
	box-sizing: border-box;
	width: 100%;
	height: 100%;
	display: flex;
